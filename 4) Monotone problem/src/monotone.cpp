...
class Solver {
    private:
        ...
        // Метод для проверки, что второй вектор по всем координатам не меньше первого. 
        bool check_order(const vector<int> &v_first, const vector<int> &v_second) const {
        ...
        bool find_match(int vertex, const vector<vector<int>>& adj_list, vector<bool> &visited, vector<optional<int>>& matching) {
            if (visited[vertex]) {
                return false;
            }
            visited[vertex] = true;
        ...
        vector<int> find_vertex_cover(const vector<vector<int>>& adj_list, const vector<optional<int>>& matching) {
            ...
            for (int neighbor = count_vectors; neighbor < 2 * count_vectors; ++neighbor) {
                if (visited[neighbor]) {
        ...
        void solve_problem() {
            ...
            for (int i = 0; i < count_vectors; ++i) {
                for (int j = i + 1; j < count_vectors; ++j) {
                    if (check_order(vectors_sorted[i].first, vectors_sorted[j].first) && vectors_data[vectors_sorted[i].second][dimension] > vectors_data[vectors_sorted[j].second][dimension]) {
                        adj_list[vectors_sorted[i].second].push_back(vectors_sorted[j].second + count_vectors);
                    }
                }
            }
