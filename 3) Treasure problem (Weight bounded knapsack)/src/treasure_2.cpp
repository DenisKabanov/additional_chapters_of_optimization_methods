// первый вариент реализации по лекции — не проходит по time limit
...
int main() {
    ...
    for (Solution right_sol : solutions_right) {
        ...
        while ((left_sol_id < solutions_left.size()) && (solutions_left[left_sol_id].weight + consumed_weight <= right_bound)) {
        ...
        if ((consumed_weight + best_left_sol.weight >= left_bound) && (consumed_weight + best_left_sol.weight <= right_bound) && (best_left_sol.price + right_sol.price > best_sol.price)) { // если решение подходит по границам и оно лучше текущего лучшего ответа
            ...
            best_sol.update_solution(consumed_weight + best_left_sol.weight, right_sol.price + best_left_sol.price, items_ids); // обновляем лучшее решение
        }
    }
    best_sol.print_solution();
