// финальный вариант без reverse и с использованием встроенной c++ сортировки — прошёл
...
int main() {
    ...
    // сортируем решения и удаляем те, чей вес одинаковый, но стоимость меньше
    std::sort(solutions_left.begin(), solutions_left.end(), [](const Solution& left, const Solution& right) {
        if (left.weight == right.weight) {
            return left.price > right.price; // decrease second value if first values are equal
        }
        return left.weight < right.weight; // increase first value
    });
    std::sort(solutions_right.begin(), solutions_right.end(), [](const Solution& left, const Solution& right) {
        if (left.weight == right.weight) {
            return left.price > right.price; // decrease second value if first values are equal
        }
        return left.weight < right.weight; // increase first value
